...
Session-only route families (\$STAR ledger, account management) are designed for browser users managing their own account and balance. They are not accessible to external agents via Bearer tokens.
...
| \$STAR ledger | Session cookie | Human (browser) |
